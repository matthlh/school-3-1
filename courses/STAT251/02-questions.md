...
**A:** (a) discrete (b) continuous (c) nominal (d) ordinal (e) nominal — the digits are labels, arithmetic on them is meaningless (f) continuous.
...
**A:** Discrete: the possible values are countable (you can list them 0, 1, 2, …), typically counts. Continuous: between any two possible values there is always another, so it can take any value in an interval — typically measurements. Rounding height to whole cm does not make it discrete; the underlying quantity decides.
...
**A:** Descriptive: methods to organise, display and summarise the data you actually have — claims about the sample / data set. Inferential: methods to draw conclusions, predictions or decisions from a sample — claims about the population (its parameters), estimated from sample statistics.
...
**A:** Bar chart: categorical data; one bar per category; gaps between bars; category order arbitrary unless ordinal. Histogram: numerical data grouped into bins; bars touch; order fixed by the number line; bin width is a choice you make.
...
**A:** Pie: one categorical variable, categories are parts of a single whole, few categories (≈ ≤ 5), and the message is "share of total". Bar chart wins when you must compare category sizes precisely, there are many categories, or you compare across groups — people judge lengths far more accurately than angles or areas.
...
**Topic:** 1b–c Displays  **Lec:** 1–2  **Type:** apply
**A:** Relative frequency = count / n; the column sums to 1 (100%). It puts data sets of different sizes on one scale so distributions can be compared, and it is the empirical version of a probability distribution — Ch 3–4 use exactly this idea.
...
**Topic:** 1a Types of data  **Lec:** 1–2  **Type:** derive
**A:** Step 1 — does arithmetic on the values (differences, averages) mean anything? No → categorical; then: natural order? No → nominal, yes → ordinal. Yes → numerical; Step 2 — countable list of possible values (counts) → discrete; any value in an interval (measurements) → continuous. Marital status: nominal. Race position: ordinal (ordered, but the gap 1st→2nd is not a fixed quantity). Finishing time: continuous.
...
**A:** Ordinal categorical. The stars are ranks, not counts of anything: 4 stars is not "twice" 2 stars and the 1→2 gap need not equal the 4→5 gap. Same for Likert scales (strongly disagree … strongly agree). Rule: numbers used as ordered labels = ordinal, not discrete.
...
**Topic:** 1a Types of data  **Lec:** 1–2  **Type:** apply
**A:** (a) ordinal (b) nominal (c) continuous (d) discrete (e) nominal (f) ordinal — a rating scale, ranks not counts.

### Q: For each variable name the display you would reach for and why: letter grade (A/B/C/D/F), study time in hours, pass/fail.
**Topic:** 1a Types of data  **Lec:** 1–2  **Type:** apply
**A:** Letter grade: ordinal → bar chart with bars in grade order (order matters, gaps between bars). Study time: continuous → histogram (bins on a number line, bars touch) or dot plot if n is small. Pass/fail: nominal, binary → bar chart (a pie is defensible with only two parts of one whole).
...
**Topic:** Descriptive vs inferential  **Lec:** 1–2  **Type:** apply
**A:** (a) descriptive — a fact about the data in hand. (b) inferential — "UBC students" names a population larger than the 40 measured. (c) descriptive. (d) inferential — "the factory's rate" is a population parameter, and the ± is the uncertainty inference must carry. Giveaway: the claim reaches beyond the units actually measured, or attaches a margin of error.
...
**Topic:** Descriptive vs inferential  **Lec:** 1–2  **Type:** recall
**A:** Population: the whole group of interest. Sample: the subset actually measured. Parameter: a number describing the population (μ, p, σ), usually unknown. Statistic: a number computed from the sample (x̄, p̂, s). Descriptive statistics works with the sample and its statistics. Inference uses a statistic to make a claim about the parameter — sample → population.
...
**A:** Descriptive: methods to organise, display and summarise the data you have (tables, graphs, mean, SD) — claims only about that data. Inferential: methods to draw conclusions about a population from a sample. It is not a guess because it quantifies its own uncertainty — a margin of error / confidence level or a p-value — computed under an explicit assumption about how the sample was drawn (e.g., random sampling).
